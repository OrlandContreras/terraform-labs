...
resource "aws_internet_gateway" "my-tflab-igw" {
  vpc_id = aws_vpc.my-tflab-vpc.id
...
resource "aws_subnet" "my-tflab-subnet" {
  vpc_id                  = aws_vpc.my-tflab-vpc.id
  cidr_block              = var.subnet_ip
  availability_zone       = var.subnet_availability_zone
  map_public_ip_on_launch = true
  depends_on              = [aws_internet_gateway.my-tflab-igw]
...
resource "aws_security_group" "my-tflab-security-group" {
...
resource "aws_network_interface" "my-tflab-network-interface" {
  subnet_id = aws_subnet.my-tflab-subnet.id
  private_ips = [
    "172.16.10.10"
  ]
  # Se asocia el Security Group con las reglas Inbound / Outbound
  security_groups = [
    aws_security_group.my-tflab-security-group.id
  ]
...
resource "aws_instance" "my-tflab-ec2" {
  ami           = var.ami_instance
  instance_type = var.instance_type

  # network_interface: Describe una interfaz de red de la instancia
  network_interface {
    network_interface_id = aws_network_interface.my-tflab-network-interface.id
    device_index         = 0
  }
...
resource "aws_eip" "my-tflab-eip" {
  domain                    = "vpc"
  network_interface         = aws_network_interface.my-tflab-network-interface.id
